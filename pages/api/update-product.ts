...
export default async function handler(
  ...
  if (id == null || contents == null) {
    res.status(400).json({ message: 'no id or cotents' })
    return
  }
  try {
    const response = await updateProduct(Number(id), contents)
    res.status(200).json({ items: response, message: `Success update product` })
  } catch (error) {
    return res.status(400).json({ message: `Failed update product` })
  }
}
